...
public class Application2 {
    ...
    public static Integer solution(String input) throws IOException {
        ...
        for (int i = 0; i < K; i++) {
            ...
            map[y][x] = 1; // 좌표와 행렬의 인덱스 개념은 반대다. (배추 심기)
        }
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) { // 배추밭의 너비부터 먼저 순회하고, 높이를 순회

                /* 설명. 방문 안한 위치에 배추가 심어져 있으면 지렁이를 뿌리자 */
                if(visit[i][j] == false && map[i][j] == 1) {
                    count++;

                    bfs(j, i); // bfs에 x, y 좌표를 각각 넘김
                }
            }
        }
        return count;
    ...
    /* 설명. 배추를 만나면 상하좌우를 둘러본다. 방문하지 않고 유효한 범위안에 또다른 배추가 심어져 있는지 확인 (방문배열 체크) */
    private static void bfs(int x, int y) {
        q.offer(new Node(x, y));
        visit[y][x] = true; // x, y 좌표체계를 배열로 바꿀려면 뒤집어야 한다 !

        while (!q.isEmpty()) {
            Node node = q.poll();

            /* 설명. que 에서 뽑은 배추가 상하좌우를 둘러보는 로직 */
            for (int i = 0; i < 4; i++) {
                cur_x = node.x + driX[i];
                cur_y = node.y + dirY[i];
